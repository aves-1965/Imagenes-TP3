...
UMBRAL_MOVIMIENTO = 4000  # Píxeles de movimiento máximo para considerarse estático. Lo bajé a 4000
FRAMES_ESTABLES = 7      # Número de frames consecutivos para confirmar el reposo

# --- PARÁMETROS AJUSTADOS PARA CORRECCIÓN ---
# Rango de área para dados más flexible
AREA_MIN_DADO = 1300      # Reducido de 1500 para capturar dados más pequeños. Lo bajé a 1300
AREA_MAX_DADO = 55000     # Aumentado de 50000 para dados más grandes. 

# Parámetros de detección de pips mejorados
UMBRAL_PIPS = 180         # Reducido de 200 para capturar pips menos brillantes
AREA_MIN_PIP = 30         # Reducido de 50 para detectar pips más pequeños

# Parámetros de morfología ajustados
DILATE_ITERATIONS = 3     # Aumentado de 2 para mejor conectividad
ERODE_ITERATIONS = 2      # Aumentado de 1 para mejor limpieza
...
def reconocer_dados(frame_estatico):
    ...
    VERSIÓN MEJORADA: Parámetros ajustados para mejor detección."""
    ...
    # Ampliado ligeramente para capturar más variaciones de rojo
    ...
    # Mejorar la máscara con Morfología (parámetros ajustados)
    ...
    for i, contorno_dado in enumerate(contornos_dados):
        ...
        if AREA_MIN_DADO < area < AREA_MAX_DADO:
            ...
            # 3. Contar Pips (Puntos Blancos) - MÉTODO MEJORADO
            ...
            print(f"   --> Dado {i+1} localizado en ({x},{y}) - Área: {int(area)} - Valor: {valor_dado}")
        else:
...
def generar_video_con_bbox(video_path, frame_idx_inicio_estatico, dados_detectados):
    ...
    output_filename = f"{base_name}_anotado_v2.mp4"
...
if frame_idx_estatico is not None:
    ...
    if ret:
        ...
        # Guardar frame estático para inspección
        cv2.imwrite(f"{VIDEO_FILE.replace('.mp4', '')}_frame_estatico.png", frame_estatico)

        # 3. Reconocimiento de Dados (Parte A) - VERSIÓN MEJORADA
        ...
        print("\n--- RESULTADOS DE LA TIRADA 1 (VERSIÓN MEJORADA) ---")
