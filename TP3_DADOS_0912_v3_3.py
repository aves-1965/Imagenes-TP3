...
AREA_MIN_DADO = 1000
AREA_MAX_DADO = 60000
NUM_DADOS_ESPERADOS = 5       # Número de dados en cada tirada

# Parámetros de detección de pips
UMBRAL_PIPS = 180
AREA_MIN_PIP = 30
VALOR_MIN_DADO = 1            # NUEVO: Mínimo valor posible en un dado
VALOR_MAX_DADO = 6            # NUEVO: Máximo valor posible en un dado
...
def reconocer_dados(frame_estatico):
    ...
    VERSIÓN 3.3: Incluye limitador de valor de dados (1-6)."""
    ...
    for i, contorno_dado in enumerate(contornos_dados):
        ...
        if AREA_MIN_DADO < area < AREA_MAX_DADO:
            ...
            # NUEVO: Aplicar limitador de valor (1-6)
            valor_original = valor_dado
            valor_dado = max(VALOR_MIN_DADO, min(valor_dado, VALOR_MAX_DADO))
            
            if valor_original != valor_dado:
                print(f"   --> [!] Candidato {i+1} en ({x},{y}): valor {valor_original} limitado a {valor_dado}")
...
def generar_video_con_bbox(video_path, frame_idx_inicio_estatico, dados_detectados):
    ...
    output_filename = f"{base_name}_anotado_v3_3.mp4"
...
if frame_idx_estatico is not None:
    ...
    if ret:
        ...
        cv2.imwrite(f"{VIDEO_FILE.replace('.mp4', '')}_frame_original_v3_3.png", frame_estatico)

        # 3. Reconocimiento de Dados - CON LIMITADOR DE VALOR
        dados_detectados_finales, frame_corregido = reconocer_dados(frame_estatico.copy())
        
        # Guardar frame CORREGIDO para comparación
        cv2.imwrite(f"{VIDEO_FILE.replace('.mp4', '')}_frame_corregido_v3_3.png", frame_corregido)

        # 4. Mostrar Resultados Finales
        valores_obtenidos = [valor for _, valor in dados_detectados_finales]
        suma_total = sum(valores_obtenidos)

        print("\n--- RESULTADOS DE LA TIRADA 1 (VERSIÓN 3.3 - VALOR LIMITADO 1-6) ---")
