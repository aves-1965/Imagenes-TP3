...
def reconocer_dados(frame_estatico):
    ...
    VERSIÓN 3.1: Incluye corrección de iluminación no uniforme."""
    
    # NUEVO: Corregir iluminación antes de procesar
    ...
    for i, contorno_dado in enumerate(contornos_dados):
        ...
        if AREA_MIN_DADO < area < AREA_MAX_DADO:
            ...
            # IMPORTANTE: usar frame_corregido para extraer ROI
            dado_roi = frame_corregido[y:y+h, x:x+w]

            # 3. Contar Pips (Puntos Blancos) - CON FRAME CORREGIDO
...
def generar_video_con_bbox(video_path, frame_idx_inicio_estatico, dados_detectados):
    ...
    output_filename = f"{base_name}_anotado_v3_1.mp4"
...
# 1. Detección de Estabilidad - MODO MEJORADO
...
if frame_idx_estatico is not None:
    ...
    if ret:
        ...
        cv2.imwrite(f"{VIDEO_FILE.replace('.mp4', '')}_frame_original_v3_1.png", frame_estatico)

        # 3. Reconocimiento de Dados (Parte A) - CON CORRECCIÓN DE ILUMINACIÓN
        dados_detectados_finales, frame_corregido = reconocer_dados(frame_estatico.copy())
        
        # Guardar frame CORREGIDO para comparación
        cv2.imwrite(f"{VIDEO_FILE.replace('.mp4', '')}_frame_corregido_v3_1.png", frame_corregido)
        ...
        print("\n--- RESULTADOS DE LA TIRADA 1 (VERSIÓN 3.1 - ILUMINACIÓN CORREGIDA) ---")
